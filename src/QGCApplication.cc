...
void QGCApplication::startUdpSender()
{
    QProcess *process = new QProcess(this);

    QString executablePath = "/home/vm-qgc/qgroundcontrol/qgroundcontrol/src/UI/UdpSender";

    process->startDetached(executablePath);

    if (!process->waitForStarted()) {
        qDebug() << "UDP sender did not start";
    } else {
        qDebug() << "UDP sender started";
    }
}

void QGCApplication::startUdpCloseSender()
{
    QProcess *process = new QProcess(this);

    QString executablePath = "/home/vm-qgc/qgroundcontrol/qgroundcontrol/src/UI/UdpCloseSender";

    process->startDetached(executablePath);

    if (!process->waitForStarted()) {
	qDebug() << "UdpCloseSender did not start";
	} else {
	   qDebug() << "UdpCloseSender started";
	}
}
...
// Register qgcApp 
    if (_qmlAppEngine) {
        ...
        qDebug() << "qgcApp registrert i qml";
    }

    else {
        qDebug() << " _qmlAppEngine er NULL";
    }
